...
setlocal
chcp 65001 > nul
title mdoc Installer
echo The mdoc installer verifies this package and installs mdoc for the current user.
echo It does not change the permanent PowerShell execution policy or bypass enterprise security policy.
echo You can install without network access by placing the verified Toolchain ZIP beside this file.
echo.
set NETWORK_ARG=
set TOOLKIT_ARG=
...
:network
choice /C YN /N /M "Allow the installer to download and verify about 400 MB of mdoc Toolchain? [Y/N] "
...
set EXIT_CODE=%ERRORLEVEL%
if not "%MDOC_INSTALL_NO_PAUSE%"=="1" pause
exit /b %EXIT_CODE%
