...
`mdoc` 是面向 Windows 10/11 x64 的多语言 Markdown 产品手册工作流工具。产品版本是 `1.3.9`；新版工作区和任务协议统一使用 `schema_version: 1`。
...
从 GitHub Stable Release 下载 `mdoc-1.3.9-windows-x64.zip`，完整解压后双击“安装 mdoc.cmd”。安装器默认安装到当前用户的 Codex skills 目录，并为 mdoc 创建独立运行环境；它不会修改外部 Python 的全局包。
