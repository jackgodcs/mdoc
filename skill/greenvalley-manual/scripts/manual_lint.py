#!/usr/bin/env python3
"""Optional, zero-install Quality Gate for GreenValley Markdown manuals."""
...
QUICK = {
    "GVMD-PATH-MISSING", "GVMD-PATH-CASE", "GVMD-PATH-ABSOLUTE", "GVMD-FILENAME-SPACE",
    "GVMD-LINK-LEVEL", "GVMD-HTML-IMG-SYNTAX", "GVMD-HTML-BLOCK-SYNTAX",
    "GVMD-HEADING-SYNTAX", "GVMD-FENCE-UNCLOSED", "GVMD-EMPHASIS-SYNTAX",
    "GVMD-PLACEHOLDER", "GVMD-IMAGE-SYNTAX", "GVMD-IMAGE-READABLE",
    "GVMD-TABLE-SYNTAX", "GVMD-TABLE-HTML",
}
FULL = QUICK | {
    "GVMD-LINK-AMBIGUOUS", "GVMD-ANCHOR-MISSING", "GVMD-ANCHOR-DUPLICATE",
    "GVMD-HTML-BLANK-LINE", "GVMD-HEADING-HIERARCHY", "GVMD-LIST-COMPAT",
    "GVMD-PARAGRAPH-LONG", "GVMD-IMAGE-WIDTH", "GVMD-IMAGE-WIDTH-STEP",
    "GVMD-IMAGE-DIMENSION", "GVMD-IMAGE-LOCALE-WIDTH", "GVMD-IMAGE-INLINE-WIDTH",
    "GVMD-BARE-URL", "GVMD-AUTOLINK-POLICY", "GVMD-PRODUCT-NAME", "GVMD-TERM-FORBIDDEN",
    "GVMD-TERM-CASE", "GVMD-TERM-INCONSISTENT", "GVMD-SPELLING", "GVMD-SPELLING-UNAVAILABLE",
    "GVMD-LOCALE-PUNCT", "GVMD-PUNCT-SPACING", "GVMD-FULLWIDTH-MIXED",
    "GVMD-QUOTE-STYLE", "GVMD-TABLE-STYLE", "GVMD-TABLE-VISUAL",
}
PROFILES = {"quick": QUICK, "full": FULL, "release": FULL}
UNSUPPRESSIBLE = {"GVMD-PATH-MISSING", "GVMD-PATH-CASE", "GVMD-PATH-ABSOLUTE", "GVMD-FENCE-UNCLOSED", "GVMD-HTML-IMG-SYNTAX"}
...
SUPPRESS_NEXT_RE = re.compile(r"<!--\s*gv-lint-disable-next-line\s+(GVMD-[A-Z0-9-]+)\s+reason=[\"']([^\"']+)[\"']\s*-->")
SUPPRESS_START_RE = re.compile(r"<!--\s*gv-lint-disable\s+(GVMD-[A-Z0-9-]+)\s+reason=[\"']([^\"']+)[\"']\s*-->")
SUPPRESS_END_RE = re.compile(r"<!--\s*gv-lint-enable\s+(GVMD-[A-Z0-9-]+)\s*-->")
DEFAULTS = {"validation": {
...
class Linter:
    ...
    def scan(self):
        for path in sorted(self.root.rglob("*.md")):
            if self.include_files is not None and relative(self.root, path) not in self.include_files:
                continue
            if " " in path.name:
                self.add(Finding("GVMD-FILENAME-SPACE", "error", "exact", relative(self.root, path), 1, "Markdown filename contains spaces", {"name": path.name}))
            self.scan_file(path)
    ...
    def scan_file(self, path: Path):
        ...
        for number, line in enumerate(lines, 1):
            ...
            if re.search(r"(?:<TODO>|\{\{[^}]+\}\}|<[^>]*(?:placeholder|insert|replace)[^>]*>)", line, re.I):
                self.add(Finding("GVMD-PLACEHOLDER", "error", "exact", rel, number, "Unresolved placeholder", {"text": line.strip()}), suppressions)
            if re.match(r"^\s*\d+\.\s+", line) and validation.get("markdown", {}).get("ordered_list_style") == "compact":
                self.add(Finding("GVMD-LIST-COMPAT", "warning", "probable", rel, number, "Ordered-list spacing may be incompatible with the configured builder", {"text": line.strip()}), suppressions)
            bare = URL_RE.search(text)
            if bare and not re.search(r"[\x60<(]https?://", line, re.I):
                self.add(Finding("GVMD-BARE-URL", "suggestion", "review", rel, number, "Bare URL may need code formatting or an explicit link", {"url": bare.group(0)}), suppressions)
            for linked in re.finditer(r"\[[^]\n]+\]\((https?://[^)\s]+)\)", line, re.I):
                if validation.get("markdown", {}).get("external_url_style", "link") == "code":
                    self.add(Finding("GVMD-AUTOLINK-POLICY", "warning", "exact", rel, number, "External URL is linked but the configured policy requires code formatting", {"url": linked.group(1)}), suppressions)
            ...
            if text.strip() and not HEADING_RE.match(line) and not line.lstrip().startswith(("- ", "* ", "+ ", "<", "|")):
                ...
                paragraph = []
        if in_fence:
            self.add(Finding("GVMD-FENCE-UNCLOSED", "error", "exact", rel, fence_line, "Unclosed fenced code block", {"start": fence_line}), suppressions)
        for rule, reason in active.items():
            self.add(Finding("GVMD-HTML-BLOCK-SYNTAX", "error", "exact", rel, len(lines), "Unclosed lint suppression block", {"rule": rule, "reason": reason}), suppressions)
        self.finish_table(rel, table_rows, suppressions)
        self.finish_paragraph(rel, paragraph, suppressions)
        self.finish_headings(rel, headings, suppressions)
        if source.count("<table") != source.count("</table>"):
            self.add(Finding("GVMD-TABLE-HTML", "error", "exact", rel, 1, "HTML table tags are unbalanced", {"open": source.count("<table"), "close": source.count("</table>")}), suppressions)
        table_style = validation.get("markdown", {}).get("table_style", "either")
        if table_style == "markdown" and re.search(r"<table\b", source, re.I):
            self.add(Finding("GVMD-TABLE-STYLE", "warning", "exact", rel, 1, "HTML table conflicts with configured Markdown table style", {"configured": table_style}), suppressions)
        if table_style == "html" and re.search(r"(?m)^\s*\|.*\|\s*$", source):
            self.add(Finding("GVMD-TABLE-STYLE", "warning", "exact", rel, 1, "Markdown table conflicts with configured HTML table style", {"configured": table_style}), suppressions)
    ...
    def check_heading(self, rel, number, line, headings, suppressions):
        ...
        if not match.group("space"):
            ...
            self.add(Finding("GVMD-HEADING-SYNTAX", "error", "exact", rel, number, "Heading marker must be followed by a space", {"text": line}, fixed, "safe"), suppressions)
    ...
    def finish_headings(self, rel, headings, suppressions):
        ...
        for line, level, title in headings:
            if previous and level > previous + 1:
                self.add(Finding("GVMD-HEADING-HIERARCHY", "warning", "probable", rel, line, "Heading level skips a level", {"previous": previous, "current": level}), suppressions)
            previous = level
            if title in seen:
                self.add(Finding("GVMD-ANCHOR-DUPLICATE", "warning", "probable", rel, line, "Duplicate heading may create an ambiguous anchor", {"title": title, "first_line": seen[title]}), suppressions)
        ...
        for match, image in nodes:
            ...
            if parsed.scheme in {"http", "https", "mailto"}:
                ...
                if policy == "code" and not image:
                    self.add(Finding("GVMD-AUTOLINK-POLICY", "suggestion", "exact", rel, number, "Configured policy requires literal URLs to use code formatting", {"target": target}), suppressions)
                continue
            if " " in target and not (raw_target.startswith("<") and raw_target.endswith(">")):
                self.add(Finding("GVMD-LINK-AMBIGUOUS", "warning", "probable", rel, number, "Link target contains an unescaped space", {"target": target}), suppressions)
            clean, _, fragment = target.partition("#")
            if ABSOLUTE_RE.match(clean):
                self.add(Finding("GVMD-PATH-ABSOLUTE", "error", "exact", rel, number, "Local absolute path is not portable", {"target": clean}), suppressions)
                continue
            requested = path if not clean else path.parent / clean
            matches = case_matches(requested)
            if len(matches) > 1:
                self.add(Finding("GVMD-LINK-AMBIGUOUS", "error", "exact", rel, number, "Local target is ambiguous under case-insensitive resolution", {"target": clean, "matches": [item.name for item in matches]}), suppressions)
                continue
            actual = matches[0] if matches else None
            rule = "GVMD-PATH-MISSING" if image else "GVMD-LINK-LEVEL"
            if actual is None:
            ...
            if written.casefold() == actual_rel.casefold() and written != actual_rel:
                self.add(Finding("GVMD-PATH-CASE", "error", "exact", rel, number, "Path case differs from the filesystem", {"written": clean, "actual": actual_rel}, actual_rel, "safe"), suppressions)
            if fragment and actual.suffix.lower() == ".md" and unquote(fragment).casefold() not in heading_anchors(actual):
                self.add(Finding("GVMD-ANCHOR-MISSING", "error", "exact", rel, number, "Markdown heading anchor does not exist", {"target": clean, "anchor": unquote(fragment)}), suppressions)
            if image:
    ...
    def check_html(self, path, rel, number, lines, line, suppressions):
        if re.search(r"<img\b[^>]*[\"']\s+[\"']\s*(?:width|height|style)=", line, re.I):
            ...
            self.add(Finding("GVMD-HTML-IMG-SYNTAX", "error", "exact", rel, number, "Image tag contains an isolated quote attribute", {"text": line.strip()}, fixed, "safe"), suppressions)
        for match in HTML_IMAGE_RE.finditer(line):
            ...
            src = attrs.get("src")
            if not src:
                self.add(Finding("GVMD-IMAGE-SYNTAX", "error", "exact", rel, number, "Image tag has no quoted src attribute", {"tag": match.group(0)}), suppressions)
                continue
            if ABSOLUTE_RE.match(src):
                self.add(Finding("GVMD-PATH-ABSOLUTE", "error", "exact", rel, number, "Image uses an absolute path", {"target": src}), suppressions)
                continue
            if re.match(r"^[a-z]+://", src, re.I):
                continue
            actual = actual_case(path.parent / unquote(src))
            if actual is None:
                self.add(Finding("GVMD-PATH-MISSING", "error", "exact", rel, number, "Image target does not exist", {"target": src}), suppressions)
                continue
            actual_rel = Path(os.path.relpath(actual, path.parent)).as_posix()
            written = Path(src).as_posix()
            if written.casefold() == actual_rel.casefold() and written != actual_rel:
                self.add(Finding("GVMD-PATH-CASE", "error", "exact", rel, number, "Image path case differs from the filesystem", {"written": src, "actual": actual_rel}, actual_rel, "safe"), suppressions)
        ...
        if re.match(r"^\s*<div\b[^>]*>\s*$", line, re.I) and self.config["validation"].get("markdown", {}).get("require_blank_line_after_html_block"):
            if number < len(lines) and lines[number].strip():
                self.add(Finding("GVMD-HTML-BLANK-LINE", "warning", "probable", rel, number, "Configured builder requires a blank line after div", {"next": lines[number].strip()}, "insert blank line", "safe"), suppressions)
        if re.search(r"</?(?:div|table|tr|td|th)\b[^>]*$", line, re.I):
            self.add(Finding("GVMD-HTML-BLOCK-SYNTAX", "error", "exact", rel, number, "HTML block tag is not closed on this line", {"text": line.strip()}), suppressions)

    def check_image(self, rel, number, src, actual, attrs, inline, suppressions):
        try:
            width, height = image_size(actual)
        except (OSError, ValueError) as exc:
            self.add(Finding("GVMD-IMAGE-READABLE", "error", "exact", rel, number, "Image header is unreadable", {"target": src, "error": str(exc)}), suppressions)
            return
        ...
        if width > limits.get("max_width_px", 8192) or height > limits.get("max_height_px", 4096) or width * height > limits.get("max_pixel_count", 16777216):
            self.add(Finding("GVMD-IMAGE-DIMENSION", "warning", "exact", rel, number, "Image dimensions exceed configured limits", {"target": src, "width": width, "height": height}), suppressions)
        ...
        if not inline and limits.get("require_width_for_block_images") and not configured:
            self.add(Finding("GVMD-IMAGE-WIDTH", "warning", "probable", rel, number, "Block image has no display width", {"target": src, "width": width}), suppressions)
        ...
        if configured and steps:
            try:
                ...
                if display not in steps:
                    self.add(Finding("GVMD-IMAGE-WIDTH-STEP", "warning", "exact", rel, number, "Image width is not a configured step", {"target": src, "width": display, "steps": steps}), suppressions)
            except ValueError:
                self.add(Finding("GVMD-IMAGE-WIDTH-STEP", "warning", "exact", rel, number, "Image width is not an integer pixel value", {"target": src, "width": configured}), suppressions)
        if inline and configured:
            try:
                if int(re.sub(r"px$", "", configured)) > 64:
                    self.add(Finding("GVMD-IMAGE-INLINE-WIDTH", "warning", "probable", rel, number, "Inline icon is unusually wide", {"target": src, "width": configured}), suppressions)
            except ValueError:
                pass
    ...
    def check_terms(self, rel, number, text, suppressions):
        for variant, canonical in self.term_variants.items():
            if variant and variant in text:
                self.add(Finding("GVMD-PRODUCT-NAME", "warning", "exact", rel, number, "Forbidden product-name variant", {"written": variant, "canonical": canonical}, canonical, "safe"), suppressions)
        for item in self.config["validation"].get("custom_patterns", []):
        ...
        for term in self.terminology:
            ...
            if expected:
                ...
                if match and match.group(0) != expected:
                    self.add(Finding("GVMD-TERM-CASE", "warning", "exact", rel, number, "Configured term uses different letter case", {"written": match.group(0), "expected": expected}, expected, "safe"), suppressions)
            for forbidden in (term.get("forbidden_variants") or "").split("|"):
                forbidden = forbidden.strip()
                if forbidden and forbidden in text:
                    self.add(Finding("GVMD-TERM-FORBIDDEN", "warning", "exact", rel, number, "Forbidden terminology variant", {"written": forbidden, "expected": expected}, expected or None), suppressions)
            other_values = {str(term.get(key) or "").strip() for key in ("source", "en", "ja") if key != locale}
            for other in other_values - {"", expected}:
                if other in text:
                    self.add(Finding("GVMD-TERM-INCONSISTENT", "suggestion", "review", rel, number, "Term from another locale appears in prose", {"written": other, "expected": expected}), suppressions)
    ...
    def check_emphasis(self, rel, number, text, suppressions):
        ...
        for marker, count in marker_counts.items():
            if count % 2:
                self.add(Finding("GVMD-EMPHASIS-SYNTAX", "error", "exact", rel, number, "Unbalanced strong-emphasis marker", {"marker": marker, "text": text.strip()}), suppressions)

    def check_language(self, rel, number, text, locale, suppressions):
        if locale == "en":
            match = re.search(r"[。，“”‘’：；、（）【】]", text)
            if match:
                self.add(Finding("GVMD-LOCALE-PUNCT", "suggestion", "review", rel, number, "English prose contains CJK punctuation", {"character": match.group(0)}), suppressions)
            if re.search(r"[，。！？][A-Za-z]", text):
                self.add(Finding("GVMD-FULLWIDTH-MIXED", "warning", "probable", rel, number, "Full-width punctuation is mixed with Latin prose", {"text": text.strip()}), suppressions)
            if re.search(r"[,;:][A-Za-z]", text):
                self.add(Finding("GVMD-PUNCT-SPACING", "suggestion", "review", rel, number, "English punctuation may need a following space", {"text": text.strip()}), suppressions)
        if locale == "ja" and "、" in text and self.config["validation"].get("punctuation", {}).get("ja") == "ascii":
            self.add(Finding("GVMD-LOCALE-PUNCT", "suggestion", "review", rel, number, "Japanese prose contains a forbidden punctuation form", {"character": "、"}), suppressions)
        quote_style = self.config["validation"].get("punctuation", {}).get("quote_style")
        if quote_style == "ascii" and re.search(r"[“”‘’]", text):
            self.add(Finding("GVMD-QUOTE-STYLE", "suggestion", "review", rel, number, "Curly quotes conflict with configured ASCII quote style", {"text": text.strip()}), suppressions)
        ...
        if spelling.get("enabled") and locale in spelling.get("required_locales", ["en"]):
            if self.dictionary is None:
                ...
                if key not in self.spelling_reported:
                    ...
                    self.add(Finding("GVMD-SPELLING-UNAVAILABLE", "warning", "exact", rel, number, "Spelling is enabled but the configured offline dictionary is unavailable", {"engine": spelling.get("engine", "wordlist"), "dictionary_file": spelling.get("dictionary_file", "")}), suppressions)
            else:
                unknown = sorted({word for word in re.findall(r"[A-Za-z][A-Za-z'-]{2,}", text) if word.casefold() not in self.dictionary})
                if unknown:
                    self.add(Finding("GVMD-SPELLING", "suggestion", "review", rel, number, "Words are absent from the configured offline dictionary", {"words": unknown[:20]}), suppressions)
        ...
        for image, entries in self.image_widths.items():
            ...
            if len(widths) > 1 and len(locales) > 1:
                for rel, locale, width, line in entries:
                    self.add(Finding("GVMD-IMAGE-LOCALE-WIDTH", "warning", "exact", rel, line, "Same-name locale images use inconsistent display widths", {"image": image, "locale": locale, "width": width, "all_widths": sorted(widths)}))
    ...
    def finish_paragraph(self, rel, paragraph, suppressions):
        if paragraph:
            ...
            if len(joined) > maximum:
                self.add(Finding("GVMD-PARAGRAPH-LONG", "suggestion", "review", rel, paragraph[0][0], "Paragraph may need manual line or step breaks", {"characters": len(joined), "lines": len(paragraph)}), suppressions)
    ...
    def finish_table(self, rel, rows, suppressions):
        ...
        if len(counts) > 1:
            self.add(Finding("GVMD-TABLE-SYNTAX", "error", "exact", rel, rows[0][0], "Markdown table rows have different column counts", {"counts": sorted(counts)}), suppressions)
        if max(counts) > 8:
            self.add(Finding("GVMD-TABLE-VISUAL", "warning", "review", rel, rows[0][0], "Wide table may not fit PDF pages", {"columns": max(counts)}), suppressions)


def write_report(linter: Linter, output: Path, profile: str, builds: list[dict] | None = None, policy: dict | None = None, visual: dict | None = None) -> dict:
...
def resolve_context(args) -> dict:
    ...
    repository_root = Path(workspace_config["repository"]["root"])
    if not repository_root.is_absolute():
        repository_root = workspace / repository_root
    formal_root = repository_root / workspace_config["manual"]["active_version"]
...
def main() -> int:
    ...
    visual = json.loads(visual_path.read_text(encoding="utf-8")) if profile == "release" and visual_path.exists() else ({"status": "not_configured"} if profile == "release" else {"status": "not_requested"})
    data = write_report(linter, output, profile, builds, policy, visual)
    if args.command == "baseline":
        ...
        baseline.write_text(json.dumps({"schema_version": 1, "fingerprints": fingerprints}, indent=2) + "\n", encoding="utf-8")
