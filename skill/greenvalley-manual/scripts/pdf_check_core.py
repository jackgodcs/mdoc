#!/usr/bin/env python3
"""Core report model and deterministic checks for GreenValley PDF Check."""
        ...
        with pdfplumber.open(pdf_path) as pdf:
            ...
            for number, page in enumerate(pdf.pages, 1):
                ...
                if number > 1 and size != common_size:
                    findings.append(finding("GVPDF-PAGE-GEOMETRY-CHANGED", "warning", "probable", artifact_id, [number], "Page size or orientation differs from the first content page", {"page_size": size, "reference_size": common_size}, scope="book-existing", blocking=False))
                if "GVVR:" in text:
                    item = finding("GVPDF-MARKER-LEAK", "error", "exact", artifact_id, [number], "Check PDF contains a GreenValley mapping marker", {"marker": "GVVR:"})
                    marker = re.search(r"GVVR:([^:]+):(\d+):\S+", text)
                    if marker:
                ...
                if not text.strip() and not images and not page.rects and not page.lines and not page.curves:
                    findings.append(finding("GVPDF-UNEXPECTED-BLANK-PAGE", "warning", "probable", artifact_id, [number], "Page has no visible text, images, or drawing objects", {"page": number}, scope="book-existing", blocking=False))
                elif len(text.strip()) < 30 and not images:
                    findings.append(finding("GVPDF-PAGE-SPARSE", "suggestion", "review", artifact_id, [number], "Page contains very little visible content", {"characters": len(text.strip())}, scope="book-existing", blocking=False))
                small_chars = []
                for char in page.chars or []:
                    if char.get("text") == "�":
                        region = {"page": number, "bbox_pt": [char.get("x0"), char.get("top"), char.get("x1"), char.get("bottom")]}
                        findings.append(finding("GVPDF-FONT-GLYPH-MISSING", "error", "exact", artifact_id, [number], "PDF text contains a replacement character", {"character": "�"}, [region]))
                ...
                if small_chars:
                    ...
                    findings.append(finding("GVPDF-TEXT-TOO-SMALL", "suggestion", "review", artifact_id, [number], "Page contains text smaller than the readable-size threshold", {"minimum_font_size_pt": min(float(char.get("size", 99)) for char in small_chars), "character_count": len(small_chars)}, [region], scope="book-existing", blocking=False))
                for word in words:
                    if word.get("x0", 0) < -1 or word.get("x1", 0) > page.width + 1 or word.get("top", 0) < -1 or word.get("bottom", 0) > page.height + 1:
                        ...
                        findings.append(finding("GVPDF-CONTENT-OUTSIDE-PAGE", "error", "exact", artifact_id, [number], "Text object extends outside the PDF page", {"text": word.get("text")}, [region]))
