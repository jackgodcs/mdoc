...
class ManualPdfCheckTests(unittest.TestCase):
    ...
    def test_existing_pdf_check_reports_blank_page_and_marker_leak(self):
        make_pdf(self.pdf, [{"texts": [("GVVR:en/Page.md:1:test", 20, 350, 10)]}, {}])
        ...
        self.assertIn("GVPDF-MARKER-LEAK", rules)
        self.assertIn("GVPDF-UNEXPECTED-BLANK-PAGE", rules)
    ...
    def test_user_ignore_removes_only_matching_effective_error(self):
        ...
        finding = next(item for item in first["findings"] if item["rule_id"] == "GVPDF-MARKER-LEAK")
        ...
        ignored = next(item for item in second["findings"] if item["rule_id"] == "GVPDF-MARKER-LEAK")
    ...
    def test_marker_mapping_and_task_scope_are_attached_to_findings(self):
        ...
        marker = next(item for item in data["findings"] if item["rule_id"] == "GVPDF-MARKER-LEAK")
    ...
    def test_small_text_is_grouped_as_one_page_finding(self):
        ...
        findings = [item for item in data["findings"] if item["rule_id"] == "GVPDF-TEXT-TOO-SMALL"]
        self.assertEqual(1, len(findings))

    def test_standard_build_adapter_receives_safe_environment_and_output(self):
        launcher = self.root / "build.py"
        launcher.write_text("import os,shutil; shutil.copy2(os.environ['TEST_PDF'], os.environ['GV_MANUAL_OUTPUT_PATH'])", encoding="utf-8")
        make_pdf(self.pdf, [{"texts": [("Page", 20, 350, 10)]}])
