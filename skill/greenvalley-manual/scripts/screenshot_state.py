...
def ensure_local_files(paths: dict[str, Path], workspace: Path, task_id: str, skill_root: Path | None = None) -> None:
    ...
    if not paths["local"].exists():
        atomic_json(paths["local"], {"schema_version": 1, "preferences": {"capture_scope": "current_monitor", "filter": "pending", "auto_advance": True, "selected_locale": independent_locales(workspace)[0]["id"], "viewed_locales": []}, "notes": {}})
    if skill_root:
