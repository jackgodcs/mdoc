(() => {
  ...
  async function previewEditor() {
    ...
    doc.querySelectorAll('p').forEach(p => { if (!p.textContent.trim() && p.children.length && [...p.children].every(child => child.tagName === 'IMG')) p.classList.add('standalone-image'); });
