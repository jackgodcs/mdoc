...
Default workspace PDFs are written below `.mdoc/artifacts/pdf/<book>/<locale>/`. Full-book default names use the locale's `book.json` title with whitespace and Windows-invalid filename characters normalized to underscores, followed by the workspace locale ID, for example `LiDAR360MLS_User_Guide_zh.pdf`; if normalization leaves no title characters, the name falls back to `<book>-<locale>.pdf`. Explicit `--output` names and page, section, or standalone naming are unchanged. Existing workspace output requires `--yes`; `--no-overwrite` skips it. Successful large intermediates are removed unless `--keep-work` is passed or the effective workspace/locale `keep_successful_book_work` setting is `true` for a full-book workspace build; explicit `--discard-work` overrides configured retention. Failed work is retained unless `--discard-work`. Cleanup removes expired workspace work, old reports, and default standalone PDFs under `%TEMP%\mdoc`; it never removes workspace final PDFs or explicitly selected standalone output paths.
