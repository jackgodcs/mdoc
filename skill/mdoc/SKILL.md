---
name: mdoc
description: Plan, capture, write, localize, validate, audit, and publish multilingual product manuals with an explicit active-book context. Use when creating or updating manual content, adding a locale, preparing screenshots, running the optional Quality Gate or PDF Check, or resuming an mdoc workspace.
---
...
Treat the GitHub repository jackgodcs/mdoc on branch main as the single source of truth for this skill and its process assets. Use a separate Git checkout as the maintenance workspace. Treat `$CODEX_HOME/skills/mdoc` as an installed runtime copy only; do not maintain long-lived changes there. Make changes under `skill/mdoc` in the checkout, validate and commit them, push them to GitHub, then install or synchronize that directory into `$CODEX_HOME/skills/mdoc`. Before new maintenance work, pull main with fast-forward-only semantics. Never copy local-only workspace bindings, `*.local.yaml`, captures, or formal manual artifacts into the GitHub source repository.
...
Generate source-locale content first, rewrite locales that require localized UI screenshots, and apply copy strategies exactly as configured. Markdown under an English `en` locale must not contain Chinese Han characters, including headings, links, HTML, inline code, or fenced code; translate or remove the text before publishing. If literal Chinese source evidence is genuinely required, use only the narrow existing reasoned suppression mechanism and keep the exception visible in Quality Gate reports. Formal manuals must not contain local absolute paths, task-workspace links, unresolved placeholders, or source-analysis implementation details irrelevant to users.
...
Read `references/screenshot-workflow.md`. Keep `screenshots.yaml` as the machine state source. Use `scripts/screenshot_state.py sync <process-workspace> <task-id>` to generate the task-local manifest and launcher, then open `.work/mdoc/<task-id>/open-screenshot-assistant.cmd`. Do not generate Markdown screenshot workbooks.
...
- `references/manual-lint.md`: Quality Gate profiles, FAQ-derived rules, baselines, suppressions, reports, fixes, and builds.
