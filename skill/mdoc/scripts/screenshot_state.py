...
The mdoc manual workspace uses a deliberately small YAML subset. This
...
def task_paths(workspace: Path, task_id: str) -> dict[str, Path]:
    ...
    work_root = workspace_repository(workspace) / ".work" / "mdoc" / task_id
    return {
        "task_dir": task_dir,
...
def ensure_local_files(paths: dict[str, Path], workspace: Path, task_id: str, skill_root: Path | None = None) -> None:
    ...
    if not paths["local"].exists():
        atomic_json(paths["local"], {"schema_version": 2, "preferences": {"capture_scope": "current_monitor", "auto_advance": True, "selected_locale": independent_locales(workspace)[0]["id"], "preview_split_ratio": 0.4, "viewed_locales": []}, "notes": {}})
    if skill_root:
...
def summary(manifest: dict) -> dict:
    ...
    return {
        ...
        "manifest": str(Path(manifest["repository"]) / ".work" / "mdoc" / manifest["task_id"] / "screenshot-assistant.json"),
    }
...
def discover(repository: Path) -> dict:
    binding = repository / ".work" / "mdoc" / "workspace-ref.local.yaml"
