#!/usr/bin/env python3
"""Zero-dependency validation for mdoc manual workspaces."""
...
VALIDATION_COMPONENTS = {"static", "html_build", "pdf_build", "pdf_check"}
...
def validate_workspace(root: Path, errors: list[str]):
    ...
    if not workspace.exists():
        ...
        if scalar(source, "schema_version") != "2":
            errors.append(f"{workspace}: schema_version must be 2")
    ...
    if not profile.exists():
        ...
        source = text(profile)
        if scalar(source, "schema_version") != "2":
            errors.append(f"{profile}: schema_version must be 2")
...
def validate_task(task_dir: Path, errors: list[str], warnings: list[str]):
    ...
    if scalar(task_source, "schema_version") != "2":
        errors.append(f"{task}: schema_version must be 2")
