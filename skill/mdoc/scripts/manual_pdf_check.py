...
def task_context(workspace: Path, task_id: str) -> dict:
    ...
    repository_value = local.get("local", {}).get("manual_repository") or config["repository"]["root"]
    ...
    target_book = task.get("task", {}).get("target_book") or config["manual"].get("active_book") or config["manual"]["active_version"]
    book = (repository / target_book).resolve()
    ...
    launcher = repository / ".work" / "mdoc" / task_id / "open-pdf-check.cmd"
