#!/usr/bin/env python3
"""Human screenshot assistant for mdoc manual tasks."""
...
class GlobalHotkey:
    ...
    def start(self):
        ...
        self.thread=threading.Thread(target=self.run,name="mdoc-global-hotkey",daemon=True); self.thread.start()
...
class Assistant:
    def __init__(self, root: tk.Tk, workspace: Path, task_id: str, check=False):
        ...
        self.root.protocol("WM_DELETE_WINDOW", self.close); self.root.title(f"mdoc 截图助手 — {task_id}")
    ...
    def accept_all(self):
        ...
        try:
            ...
            try: state.accept(self.workspace,self.task_id); self.refresh(); prompt=f"使用 $mdoc 继续 {self.task_id}，截图已完成并已由用户目视接受，请同步状态并继续后续流程。"; self.root.clipboard_clear(); self.root.clipboard_append(prompt); messagebox.showinfo("已接受","已记录总体验收，并将继续任务提示复制到剪贴板。",parent=self.root)
